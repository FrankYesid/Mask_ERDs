...
% filtrado de cad sujeto.
for s = 4
    for k = 1:numel(filter_bank)
%         fprintf(['Sujeto: ' num2str(k) ' de 9' '\n'])
        Xfreq = fcnfiltband(X{s},fs,filter_bank(k,:),5);           % funcion Filtro.
        A{s}{k} = Xfreq;
    end
end
X = A;
...
noverlap = floor(0.99*Nw);                        % interpolación de la ventana.
X_suj = cell(9,1);                                % celdas almacenadoras de spectrogram.
Clase = [1];                                      % clase o clases seleccionadas.
tic
for su = 1:3                                      % calculo del espectrograma relacionada a EEG.
...
%     temp = size(temp,2);
    ...
    for tri = 1:N_trial
        ...
        diary('C:\Users\Lufe\Desktop\reporte.txt')
    end
end
...
% rango de amplitud
Lrang = [-1.5 1.5];                                                             % rango de colocar la señal.
% graficador en 2D todos los canales
for sub = 1:9
    i = 1;
    figure
    for ch = 1:22
        ...
        imagesc(t',f,squeeze(ERD{sub}(ch,:,:)))%,[Lrang(1) Lrang(2)])
        title(['Ch: ' num2str(ch)])
        xlabel('time (seg)')
%         ylim([1 40])
        xlim([1 t(end)])
