...
%% Limpiar
clear; clc
% limpia la ventada de comandos y también la memoria del espacio de trabajo.
%% Load Database
% load('BCICIV_2a.mat');                          % carga base de datos EEG. BCICIV_2a
% load('labels.mat');                             % carga base de datos de los canales mas relevantes.
% load('layout');                                 % carga base de datos.
%% Load GIGASCIENCE    peores [17,38,16,40,8] - buenos [43,14,50,3,41].
Xx = cell(52,1);
yy = cell(52,1);
X_suj = cell(52,1);
for s = [43,14,50]
     load(['F:\GIGASCIENCE_' num2str(s) '\eeg\raw.mat'])
    Xx{s} = X;
    yy{s} = y;
    
    ns = numel(Xx);
    N_channels = size(X{1},2);
    %% Filter Band
    % clc;
    ...
    % %%
    % for fil = 11:15
    %     a = [fil+1 fil+3];                          % rango del filtro.
    %     A = cell(9,1);                              % celdas de la señal filtrada.
    %     % filtrado de cad sujeto.
    %     for k = 1:9
    ...
    Clase = [1 2];                                  % clase o clases seleccionadas.
    for su = s                                    % calculo del espectrograma relacionada a EEG.
...
%% mean ERD
for s = [3,8,14,16,17,38,40,41,43,50]
    load(['X_suj_' num2str(s) '.mat'])
    fprintf(['Subject ' num2str(s)])
    r_nc = squeeze(mean(X_suj{s}(:,:,:,ul:up),4));
    r_c = squeeze(mean(r_nc,1));
    m_c = squeeze(mean(X_suj{s},1));
    ERD{s} = bsxfun(@times,m_c,1./r_c) - 1;
end
