...
%   F. Y. Zapata C. 2019
%%
function Graficas_ERDs_channels_all2(r,erdcolormap,name,sav,Dir_save,clas)
...
addpath(genpath('G:\Dropbox\ERD\results_ERDfc_subjects\Biosig_ERD\biosig\t310_ERDSMaps\'))
%% Output parameters:
...
for i_rows = 1:n_rows
    ...
    for i_cols = 1:n_cols
        if sum(counter_total == plot_index) == 1
            ...
            %             else
             imagesc(0:1/250:7, 1:17, r{fold}{clas}{(counter_plots11)},[-1,1.5]);%imagesc(r{fold}{clas}.t_plot, r{fold}{clas}.f_plot, r{fold}{clas}.ERDS{counter_plots11}.erds', [-1, 1.5]);
            %             end;
            ...
            if counter_total == 4 || counter_total == 9 || counter_total == 15 || counter_total == 23 || counter_total == 31 || counter_total == 39
                ...
                %                 if labels ==1
                a{i_rows,i_cols}.YTick=([1:2:17]);
                a{i_rows,i_cols}.YTickLabel = num2str([filter_bank(a{i_rows,i_cols}.YTick,:)]);
...
if sav == 1
    ...
    saveas(gca,[Dir_save,'ERD_sub',num2str(name),'class',num2str(clas),'_3_4'],'epsc')
    saveas(gca,[Dir_save,'ERD_sub',num2str(name),'class',num2str(clas),'_3_4'],'png')
end
