...
%   F. Y. Zapata C. 2020
%%
function [maxi,mini]=Graficas_ERDs_channels_giga3(r,erdcolormap,labels,labels2,name,sav,Dir_save)
...
set(gcf,'position',[267   328   1300   600])
plot_index =1:numel(channels);
...
%             r{fold}{clas}.ERDS{plot_index(counter_plots11)}.erds(xx,yy) = -1;
                   ...
                   imagesc(-2:1/512:5,1:17, r{fold}{clas}.ERDS{channels(counter_plots11)}.erds, [-1, 1.5]);
...
%             title('C$_{4}$','FontSize',15,'Interpreter','latex')
        ...
        if clas == 2
            ...
            if counter_plots == 1 || counter_plots == numel(channels)+1
...
%         end;
        ...
        line([0.5, 0.5], [v(3), v(4)], 'LineStyle', ':', 'Color', 'k','LineWidth',1);
